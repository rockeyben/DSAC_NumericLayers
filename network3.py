...
# sampling num N = 100
hyp_num = HYPNUM
sampling3D1 = np.random.uniform(0, 20, size=(SAMPLESIZE, 3))
sampling2D1 = np.random.uniform(0, 5, size=(SAMPLESIZE, 2))
objIdx1 = np.random.randint(SAMPLESIZE, size=(hyp_num, 4))
objIdx1 = np.cast['int32'](objIdx1)
shuffleIdx1 = np.zeros((8, SAMPLESIZE), dtype=np.int)
...
diffMaps1 = np.zeros((hyp_num, SAMPLESIZE))
#for i in range(1):
